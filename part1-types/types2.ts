...
// 當然也可以同時指定結構跟賦值
let object4: { name: string; age: number } = { name: "julia", age: 22 };


// ------------------------------ 2. 指定 function 結構，以及參數與返回值的型別 ------------------------------
let fn1: (a: number, b: number) => number;
fn1 = function (a: number, b: number): number {
  return a + b;
};
...
// 亦可以用於表示一個數據的元數據，例如：數據庫表格中的一行數據
type Row = [number, string, boolean]; // type 宣告的用法會在稍後提到
const row: Row = [1, "Product A", true];


// ------------------------------ 5. 列舉 enum：用於取值被限定在一定範圍內的情況，比如一週只能有七天，顏色限定為紅綠藍等 ------------------------------
// 列舉成員會被從 0 開始遞增賦值，也可以自己賦值
enum Gender {
  male,
  female,
}
...
let i: { name: string; gender: Gender } = { name: "alan", gender: Gender.male };
